// Maskot komponent — reprezentácia páru S+M
// 3 varianty cez Tweaks: 'pebbles' (default) / 'sprouts' / 'orbits'

const Mascot = ({ variant = 'pebbles', size = 60, animated = true, mood = 'idle' }) => {
...
// --- Default: two pebbles cuddling, each with little dot eyes ---
const MascotPebbles = ({ size = 60, animated = true, mood = 'idle' }) => {
  ...
  return (
    <svg viewBox="0 0 80 70" style={{ width: size, height: size * (70 / 80), overflow: 'visible' }}>
      ...
      {/* right (back) pebble — slightly taller, behind */}
      <g style={animated ? { transformOrigin: '52px 40px', animation: 'mascotBobR 4.4s ease-in-out infinite' } : {}}>
        <path
          d="M30 32 Q30 12 50 12 Q70 12 70 32 Q70 58 50 58 Q30 58 30 32 Z"
          fill="url(#mascot-shade-r)"
        />
        {/* eyes */}
        {sleeping ? (
          <>
      ...
      <g style={animated ? { transformOrigin: '24px 42px', animation: 'mascotBobL 4.4s ease-in-out infinite' } : {}}>
        <path
          d="M5 38 Q5 18 24 18 Q44 18 44 38 Q44 60 24 60 Q5 60 5 38 Z"
          fill="url(#mascot-shade-l)"
        />
        {/* eyes */}
        {sleeping ? (
          <>
...
// --- Sprouts variant: two leaves on one stem ---
...
// --- Orbits variant: two interlocking rings, letters in their own space, heart in the shared overlap ---
const MascotOrbits = ({ size = 60, animated = true }) => (
  <svg viewBox="0 0 90 70" style={{ width: size * (90 / 70), height: size, overflow: 'visible' }}>
    <defs>
      ...
      {/* clip to the lens where the two rings overlap */}
      <clipPath id="orbit-lens">
        <circle cx="34" cy="35" r="21" />
      </clipPath>
    </defs>

    <ellipse cx="45" cy="63" rx="26" ry="2.5" fill="rgba(20, 30, 22, 0.1)" />

    {/* soft central glow behind the heart */}
    ...
    {/* two rings orbiting around the shared centre */}
    ...
    {/* heart nested in the shared overlap */}
...
Object.assign(window, { Mascot, MascotPebbles, MascotSprouts, MascotOrbits });
