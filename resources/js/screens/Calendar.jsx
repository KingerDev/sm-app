// Kalendár — „dôležité dni" overlay: mesačná mriežka + čoskoro + nová udalosť (podľa design/hifi-calendar.jsx)
import { cloneElement, useMemo, useState } from 'react';
import { api } from '../api';
import { useStore } from '../store';
import { Icons, Sheet } from '../components/shell';
import { MONTHS_SK, parseDate, today, toInputDate } from '../lib/dates';
...
export default function Calendar({ onBack, navigate }) {
    const { events, refresh } = useStore();
    const TODAY = today();

    const evs = useMemo(
        () => events.map(e => ({ ...e, d: parseDate(e.date) })),
        [events],
    );
    ...
    // Mriežka (pondelok prvý)
    const first = new Date(year, month, 1);
    const startOffset = (first.getDay() + 6) % 7; // 0 = pondelok
    ...
    const eventsOn = (d) => evs.filter(e => sameDay(e.d, d));
    const upcoming = evs.filter(e => e.d >= TODAY).slice(0, 5);
    const selectedEvents = eventsOn(selected);

    const shift = (delta) => setCursor(new Date(year, month + delta, 1));

    const deleteEvent = async (e) => {
        if (!confirm(`Vymazať „${e.title}"?`)) return;
            ...
            <div className="scroll" style={{ flex: 1 }}>
                ...
                <div className="row between" style={{ alignItems: 'center', marginBottom: 12 }}>
                    <button className="icon-btn" onClick={() => shift(-1)} style={{ width: 34, height: 34 }}>
                        {cloneElement(Icons.back, { style: { width: 18, height: 18 } })}
                    </button>
                                ...
                                <div style={{ display: 'flex', gap: 2, height: 4 }}>
                                    {dayEvents.slice(0, 3).map((e, j) => (
                                        <span key={j} style={{
                                            ...
                                            background: isSel ? 'var(--paper)' : (KIND_COLOR[e.kind] || 'var(--green)'),
                                        }} />
                                    ))}
                                </div>
                            </button>
                        ...
                        <div className="col gap-8">
                            {selectedEvents.map((e, i) => (
                                <div key={i} className="row gap-10" style={{ alignItems: 'center' }}>
                                    ...
                                    {e.is_custom && (
                                        <button className="btn ghost" style={{ padding: 4, color: 'var(--muted-2)', flexShrink: 0 }}
                                            onClick={() => deleteEvent(e)}>
                                            {cloneElement(Icons.trash, { style: { width: 15, height: 15 } })}
                                        </button>
                                    )}
                                </div>
                            ))}
                ...
                <div className="col gap-8">
                    {upcoming.map((e, i) => {
                        const days = Math.ceil((e.d - TODAY) / 86400000);
                        ...
                        return (
                            <button key={i}
                                ...
                                <div style={{
                                    ...
                                    background: 'var(--green-soft)', color: KIND_COLOR[e.kind] || 'var(--green)', lineHeight: 1,
                                }}>
                                    <span style={{ fontSize: 15, fontWeight: 700 }}>{e.d.getDate()}</span>
                                    <span style={{ fontSize: 8.5, textTransform: 'uppercase', letterSpacing: 0.3 }}>
                                        {MONTHS_SK[e.d.getMonth()].slice(0, 3)}
                                    </span>
                                </div>
                                ...
                                {clickable ? <span style={{ color: 'var(--muted-2)', flexShrink: 0 }}>{Icons.arrow}</span> : null}
                            </button>
                        );
                    })}
                </div>
...
function EventAdd({ initial, onClose, onSaved }) {
    const { refresh } = useStore();
    const TODAY = today();
    const startDate = initial && initial >= TODAY ? initial : TODAY;

    const [title, setTitle] = useState('');
    const [dateStr, setDateStr] = useState(toInputDate(startDate));
    ...
    const future = parsed && parsed >= TODAY;
    const canSave = title.trim() && parsed && future && !busy;
        ...
        <Sheet onClose={onClose}>
            ...
            <div style={{ marginTop: 10 }}>
                ...
                <div className="eyebrow" style={{ marginBottom: 10 }}>
                    poznámka <span style={{ textTransform: 'none', letterSpacing: 0, color: 'var(--muted-2)' }}>· volitelné</span>
                </div>
                ...
                {error && (
                    <div style={{ marginTop: 8, fontSize: 12.5, color: 'var(--accent)', textAlign: 'center' }}>{error}</div>
                )}
            </div>

            <button className="btn primary" disabled={!canSave} onClick={save}
                style={{
                    width: '100%', marginTop: 14, padding: '15px', fontSize: 15,
                    opacity: canSave ? 1 : 0.45, cursor: canSave ? 'pointer' : 'default',
                }}>
                {busy ? 'ukladám…' : 'pridať udalosť'}
            </button>
        </Sheet>
    );
}
